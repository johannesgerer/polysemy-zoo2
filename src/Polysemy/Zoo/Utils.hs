...
collectErrors f = fmap (first toList . containerPartitionEithers) . mapM (try . f) 
  >=> \(errs, vals) -> vals <$ when (not $ null errs) (throw $ unlinesT errs)
...
collectErrors2 a b = do
  ...
  bE <- try b
  case (aE, bE) of
    (Left ae, Left be) -> throw $ unlinesT [ae,be]
    _ -> fromEither $ (,) <$> aE <*> bE
...
lookupErrorNoKeys = lookupError' $ Just 0
{-# INLINE lookupErrorNoKeys #-}

lookupErrorAllKeys = lookupError' $ Just maxBound
...
deriving instance FromJSON Severity
