...
lookupError'
  :: (TE r, Show b, FoldableWithIndex b t, Lens.Ixed (t a), Typeable b, Lens.Index (t a) ~ b) =>
     Maybe Int -> Text -> b -> t a -> Sem r (Lens.IxValue (t a))
lookupError' showKeys msg k = fromEither . lookupThrow' showKeys msg k
{-# INLINEABLE lookupError' #-}

lookupErrorNoKeys = lookupError' Nothing
{-# INLINE lookupErrorNoKeys #-}
lookupErrorKeys = lookupError' (Just maxBound)
...
-- | λ> either putStrLn print =<< runM (runError $ checkAbsError "error:\n" 0.01 100 102)
--   error:
--   abs(102.0 - 100.0)
--    = 2.0 > 1.0e-2
checkAbsError :: (Ord a, Num a, TE r, Show a) => Text -> a -> a -> a -> Sem r ()
checkAbsError msg tol expected actual = when (diff > tol) $ throw $ msg <>
...
-- | λ> either putStrLn print =<< runM (runError $ checkRelError "error:\n" 0.01 100 102)
--   error:
--   abs(102.0 - 100.0)
--    = 2.0 > 1.01
--    = 1.0e-2 * 101.0
checkRelError :: (Fractional a, Ord a, TE r, Show a) => Text -> a -> a -> a -> Sem r ()
checkRelError msg tol expected actual = when (diff > tol * avg) $ throw $ msg <>
  [qq|abs({actual} - {expected})
= {diff} > {tol * avg}
...
liftYamlDecode :: forall c r . (HasCallStack, Member (Embed IO) r,  TW r, FromJSON c)
  => ByteString -> Sem r c
liftYamlDecode path = do
  (w,r) <- withFrozenCallStack eitherErrorShowM <=< embed $ (Y.decodeHelper_ . LY.decode) path
  r <$ tell (shot <$> w)

-- | Will throw error on DuplicateKey-warning
liftYamlDecodeFile :: forall c r . (HasCallStack, Member (Embed IO) r,  TW r, FromJSON c)
  => FilePath -> Sem r c
liftYamlDecodeFile path = do
  (w,r) <- withFrozenCallStack eitherErrorShowM <=< embed $ Y.decodeFileWithWarnings path
  r <$ tell (shot <$> w)
