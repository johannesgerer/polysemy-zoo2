...
import           Polysemy.Time hiding (second, hour)
...
blockCacheTests (selectedApi,usePythonApi) sev keep clear dir = do
  runBlockCacheInIOConstant (Just sev) "2020-02-05 10:00" ("1990-01-01" :: Day, mempty :: Map EntityKey Int) $
   runPythonInEmbedIONoCache (PythonConfig "python" Nothing) $ do
    openCacheConnection sev clear dir d1 Nothing
    ...
    let sp4 = do np $ it "should request both" $ state4 `shouldBe` addCounts state3 (fromList [("ent1", 1), ("ent2", 1)])
                 it "should results should be correct" $ fromList r4 `shouldBe` intvres
                 np $ it "should update from a certain date" $ startDate `shouldBe` "2020-02-03"
        intvres = S.fromList [("ent1","2020-02-06","2020-02-06",1058885.0,"ent1: 2020-02-06CloseH")
                             ,("ent1","2020-02-07","2020-02-07",1058886.0,"ent1: 2020-02-07Close")
                             ,("ent2","2020-02-05","2020-02-05",2058884.0,"ent2: 2020-02-05CloseH")]
    ...
    where reqx includeDataDate l tr = BlockCacheRequest Info dir (putStrLn <$ guard tr)
            ...
            (KeepTempTables keep) (IncludeDataDateColumn includeDataDate) (UpdateAllEntities False) l
          reqx2 l tr = BlockCacheRequest Info dir (putStrLn <$ guard tr) (fromList [scale 17 hour, scale 35 hour])
                      (KeepTempTables keep) (IncludeDataDateColumn True) (UpdateAllEntities False) l
          req = reqx True $ ExactDateRequest (ForwardFillDays 3)
            [("2020-02-04","ent1")
            ,("2020-02-03","ent2")
            ,("2020-02-07","ent2")
            ,("2020-02-09","ent2")]
...
-- devmain = hspec . describe "BlockCache" . sequence_  =<<
...
pythonApi = pythonSource $ \req -> fmap ([],) $ do
  (m, end) <- helper
  -- print True
  pure $ [qc|
def testRow(d,m,e):
...
blockApi = haskellSource $ mapM rawBlockData . fmap (second toList)

rawBlockData :: (Members '[GhcTime, S.State (Day, Map EntityKey Int), Embed IO] r)
  => (Day, [EntityKey]) -> Sem r [D1Cols]
rawBlockData (start, ents) = do
  (m, end) <- helper
  -- print (t, ents)
