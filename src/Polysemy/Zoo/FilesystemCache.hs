...
fsCachedSerialisable :: (Typeable a, Serialisable a, Members '[ErrorE] r, FsCache r)
  => FsCacheKey -> Sem r a -> Sem r a
fsCachedSerialisable ck = withFrozenCallStack $ unsafeFsCached ck (fromEither . toSerialise)
  $ fromEitherWith shot . fromSerialise . toS
...
fsMultiCachedSerialised :: (Typeable a, Serialise a, Members '[ErrorE] r, FsCache r) =>
  FsCacheBucket -> [(b, Md5Hash)] -> MultiGenerator w b a (Sem r) -> Sem r (w,[(b, a)])
fsMultiCachedSerialised b a = withFrozenCallStack $ unsafeFsMultiCached b a (pure . toS . S.serialise)
  $ fromEitherWith shot . S.deserialiseOrFail . toS
{-# INLINE fsMultiCachedSerialised #-}

toSerialise :: Encodable a => a -> Either ErrorMsg ByteString
toSerialise = fmap (toS . S.serialise) . ensureEncodable

fromSerialise :: Decodable b => LByteString -> Either ErrorMsg b
fromSerialise = left shot . fmap fromDecodeOnly . S.deserialiseOrFail

fsMultiCachedSerialisable :: (Typeable a, Serialisable a, Members '[ErrorE] r, FsCache r) =>
  FsCacheBucket -> [(b, Md5Hash)] -> MultiGenerator w b a (Sem r) -> Sem r (w,[(b, a)])
fsMultiCachedSerialisable b a = withFrozenCallStack $ unsafeFsMultiCached b a (fromEither . toSerialise)
  $ fromEitherWith shot . fromSerialise . toS
{-# INLINE fsMultiCachedSerialisable #-}
