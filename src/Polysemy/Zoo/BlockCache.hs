...
data BlockCacheRequest = BlockCacheRequest
  ...
  , bcForceUpdateTodays           :: Bool
  -- ^ this is done by adding the current time to the list of thresholds and will therefore force update
  -- if today's data is requested, but possibly also when older data is requested (however at most once per day)
  , bcThresholds                  :: NonEmpty Timespan
  -- ^ UTC thresholds.
  -- 
  -- Roughly: if current UTC time passes these thresholds the data will be refreshed.
  -- 
...
data MissingDataHandling = ForwardFillDays (Bool -- ^ True: treat whole row as missing if any value column is null
                                           -- False: only forward-fill for actually missing rows
                                           , Maybe Int) | LeaveMissing | CutoffDate Day
...
blockCachedTable desc req source =  withFrozenCallStack blockCached desc req source tableResultWithErrors
...
blockCachedConsumeWithErrors desc req source (init, nextRow) = blockCached desc req source $
...
blockCachedConsume :: (HasCallStack, SourceConn conn, Member BlockCache r, FromRow resultRow)
  => BlockCacheDescr -> BlockCacheRequest -> BlockSource conn (Sem r) -> DataConsumer res resultRow
  -> Sem r res
blockCachedConsume desc req source (init, nextRow) = blockCached desc req source $
...
runBlockCache = interpretH $ \case
  BlockCached desc re getBlocksAndIngest resultAction -> do
    ins <- getInspectorT
...
resultQuery tt desc br conn = ("SELECT " <>) <$> case bcPayload br of
  ...
  ExactDateRequest     (CutoffDate (Day c)) _ -> pure 
    [qc|r.bcKey, r.bcDate{dd "d.bcDate"}{colList "d." desc} FROM {tRequest tt} r
       INNER JOIN entities e ON e.bcKey = r.bcKey
...
assembleBlockSourceRequest sconn conn updateTime desc bc@BlockCacheRequest{..} tt@TempTables{..} = do
  ...
  count <- measureLog (sev1 bc) (\n -> "Marked " <> shot n <> " entities to be updated")
    $ embedCatch $ entitiesToBeUpdated conn tt updateTime thresholds bcPayload
  ...
  if count == 0 then pure Nothing else
    let toScR (Only minLastUpdate) = BlockSourceRequest (sev1 bc) sconn (getCutoffDate bcPayload)
          (coerce tNewData) desc $
          [(bdStartDate desc, "SELECT bcKey FROM " <> tUpdateRequired <> " WHERE bcLastUpdate IS NULL")]
          <> ffor (toList @Maybe minLastUpdate)
          (\mlu -> (add 1 $ timeToDayTruncate $ maximum thresholds `substr` mlu
...
-- min {d| u < d + t for any t and u} = min {d| min(u) < d + max(t)} = floor(min(u) - max(t)) + 1
    ...
    in fmap (fmap toScR . headMaybe) $ embedCatch $ query_ conn $ "SELECT MIN(bcLastUpdate) FROM "
       <> tUpdateRequired
  where thresholds = bool id (timeToTimespanSinceMidnight updateTime NE.<|)
                     bcForceUpdateTodays bcThresholds
...
entitiesToBeUpdated :: (Foldable f, Functor f)
  => Connection -> TempTables -> Time -> f Timespan -> BlockCacheRequestPayload -> IO Int
entitiesToBeUpdated conn tt updateTime thresholds request = withTransaction conn
  ...
  where
    q = Query $ foldr' (\a b -> b <> "\n  OR " <> a)
      [qc|INSERT INTO {tUpdateRequired tt} (bcKey, bcLastUpdate)
         ...
         e.bcLastUpdate IS null |] (condition <$> thresholds)
