...
data Memoize m a where
  ...
  UnsafeMemoizeMulti    :: (HasCallStack, MemoizeMultiContainer f, Typeable a)
    ...
    --
    -- Cache misses not returned will also be inserted into the cache as Nothings
    -> Memoize m (w, f (b,a))
...
multi ignoreCache sev msgM loc keyF inputs defW action = do
  ...
  if null recalc then pureT (defW, hits) else do
    (w, res) <- fmap (second $ extract2 $ keyF . fst) $ measureLog sev
...
devmain = runMemoizedForeignStoreInIO $ runMemoizedWithModTime Info Nothing (MemoizeKey ())
  (pure "/tmp/a") $ embedCatch $ B.readFile "/tmp/a"

devmain2 = runMemoizedForeignStoreInIO $ runMemoizedWithModTime Info Nothing (MemoizeKey ())
  (pure "/tmp/a") $ embedCatch $ B.readFile "/tmp/a"
